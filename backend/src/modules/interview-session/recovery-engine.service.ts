...
export class RecoveryEngineService {
     ...
     */
    public static async validateAndRestore(token: string, fingerprint: string, ip: string, userAgent: string): Promise<any> {
        ...
        try {
            ...
            if (result.rows.length === 0) throw new EnterpriseError("Invalid or non-existent token", "SESSION_NOT_FOUND", 404);
     ...
     */
    public static async createSnapshot(sessionId: number, trigger: string): Promise<void> {
