...
export class BulkInviteService {
   ...
   */
  public static async createBulkInviteJob(
    userId: number | null,
    name: string,
    assessmentId: number,
    jobId: number | null,
   ...
   */
  public static async processPendingInvites(batchSize = 5) {
    const client = await pool.connect();
    try {
      ...
      for (const row of candidatesRes.rows) {
        ...
        try {
          ...
          const assessRes = await client.query(
            "SELECT duration_minutes, title FROM assessments WHERE assessment_id = $1",
            [assessmentId]
          );
          ...
          const candRes = await client.query(
            "SELECT candidate_id FROM candidates WHERE email = $1 LIMIT 1",
            [email]
          );
          if (candRes.rows.length > 0) {
            ...
            const newCand = await client.query(
              "INSERT INTO candidates (full_name, email, phone, created_at) VALUES ($1, $2, $3, NOW()) RETURNING candidate_id",
          ...
          await client.query(
            `UPDATE bulk_invite_candidates SET status = 'sent', error_message = NULL, updated_at = NOW() WHERE id = $1`,
            [candidateIdInTable]
          );

          // Update success count on job
          await client.query(
            `UPDATE bulk_invite_jobs SET success_count = success_count + 1, updated_at = NOW() WHERE id = $1`,
            [jobId]
          );
      ...
      const activeJobIds = Array.from(new Set(candidatesRes.rows.map((row: any) => row.bulk_invite_job_id)));
      for (const jid of activeJobIds) {
        ...
        if (checkRes.rows.length > 0 && checkRes.rows[0].pending_count === 0) {
          await client.query(
            `UPDATE bulk_invite_jobs SET status = 'completed', updated_at = NOW() WHERE id = $1`,
            [jid]
          );
        }
      }
   ...
   */
  private static async markInviteFailed(
    client: PoolClient,
    candidateTableId: number,
    jobId: number,
    email: string,
    name: string,
    errorMsg: string
    ...
    await client.query(
      `UPDATE bulk_invite_candidates SET status = 'failed', error_message = $1, updated_at = NOW() WHERE id = $2`,
      [errorMsg, candidateTableId]
    );

    await client.query(
      `UPDATE bulk_invite_jobs SET failed_count = failed_count + 1, updated_at = NOW() WHERE id = $1`,
      [jobId]
    );
   ...
   */
  public static async getBulkInviteJobs(userId: number | null, userRole: string, page = 1, limit = 10) {
   ...
   */
  public static async getBulkInviteJobDetail(jobId: number) {
   ...
   */
  public static async retryFailedInvites(jobId: number) {
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      ...
      const checkRes = await client.query(
        "SELECT COUNT(*)::int FROM bulk_invite_candidates WHERE bulk_invite_job_id = $1 AND status = 'failed'",
        [jobId]
      );
