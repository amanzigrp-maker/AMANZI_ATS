...
export const sendInterviewLink = async (to: string, name: string, loginUrl: string, password?: string): Promise<void> => {
  try {
    ...
    console.log('\n==============================================');
    console.log('📧 [FALLBACK] Interview Login Details');
    console.log('==============================================');
    console.log(`To: ${to}`);
    console.log(`Login URL: ${loginUrl}`);
    console.log(`Password: ${password}`);
...
export const sendInterviewResults = async (
  ...
  try {
    ...
    const timeTakenStr = timeTakenMins != null ? `${timeTakenMins} minute${timeTakenMins !== 1 ? 's' : ''}` : 'N/A';
    ...
    if (!transporter) {
      ...
      console.log(`Performance Analysis:`, breakdown);
      console.log(`==============================================\n`);
      return;
    }
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <head>
          <style>
            ...
            .score-card { background: linear-gradient(135deg, #f8fafc 0%, #eef2ff 100%); border: 1px solid #e0e7ff; border-radius: 12px; padding: 24px; text-align: center; margin: 20px 0; }
            .score-big { font-size: 48px; font-weight: 800; color: #4F46E5; margin: 8px 0; letter-spacing: -1px; }
            .score-label { font-size: 13px; color: #6b7280; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; }
            .perf-bar { background: #e5e7eb; border-radius: 8px; height: 12px; margin: 16px 0 8px; overflow: hidden; }
            .perf-fill { height: 100%; border-radius: 8px; transition: width 0.5s; }
          ...
          <div class="wrapper">
            <div class="container">
              <div class="header">
                ...
                <p>${role ? `Technical Results — ${role}` : 'Technical Results'}</p>
              </div>
              <div class="content">
                <p>Dear <strong>${name}</strong>,</p>
                <p>Thank you for taking the assessment. Your results have been processed and analyzed. Here is your scorecard:</p>

                <div class="score-card">
                  <div class="score-label">Final Score</div>
                  <div class="score-big">${score} / ${total}</div>
                  <div style="font-size: 16px; color: ${performanceColor}; font-weight: 700; margin-bottom: 4px;">
                    ${performanceEmoji} ${performanceLabel} — ${percentage}%
                  </div>
                  <div class="perf-bar">
                    <div class="perf-fill" style="width: ${percentage}%; background: ${performanceColor};"></div>
                  </div>
                </div>
                ...
                <div style="background: #fdf2f2; border: 1px solid #fee2e2; border-radius: 10px; padding: 20px; margin: 24px 0;">
                  ...
                  <p style="margin: 0; color: #b91c1c; font-size: 14px;">
                    Your profile is now under evaluation. <strong>If selected, you will receive a follow-up email regarding the next steps of the interview process shortly.</strong>
                  </p>
                </div>
      ...
      `,
      text: `
Assessment Results — Amanzi

Dear ${name},

Thank you for taking the assessment. Your results have been shared with the recruitment team for review.

YOUR SCORE: ${score} / ${total} (${percentage}%)
Performance: ${performanceLabel}

WHAT HAPPENS NEXT?
- If selected, you will receive a mail regarding next steps.
...
export const sendSelectionEmail = async (to: string, name: string, role?: string): Promise<void> => {
  try {
    ...
    if (!transporter) {
      console.log(`\n==============================================`);
      console.log(`📧 [DEV MODE] Selection Notification`);
      console.log(`==============================================`);
      console.log(`To: ${to}`);
      console.log(`Name: ${name}`);
      console.log(`Message: Congratulations on being selected for ${role}!`);
      console.log(`==============================================\n`);
      return;
    }
