 ...
 * Provides structured, production-safe logging with:
...
const getLogLevel = (): LogLevel => {
  ...
  const validLevels: LogLevel[] = ["silent", "fatal", "error", "warn", "info", "debug", "trace"];
...
const redactSensitiveData = (value: any): any => {
  if (typeof value === "string") {
    return value.replace(
      /(password|otp|token|secret|apikey|api_key)\s*[:=]\s*[^\s,}]+/gi,
...
export class StructuredLogger {
   ...
   * Set user context
   */
  withUser(userId: number | string, email?: string, role?: string): StructuredLogger {
   ...
   * Set request context
   */
  withRequest(method: string, path: string): StructuredLogger {
    return this.withContext({
      method,
      path,
    });
   ...
   */
  withInterview(interviewId: string, candidateId?: string): StructuredLogger {
    return this.withContext({
      interviewId,
      candidateId,
    });
   ...
   */
  withSocket(socketId: string, room?: string): StructuredLogger {
    return this.withContext({
      socketId,
      room,
    });
  ...
  logEvent(
    eventName: string,
    eventType: "audit" | "security" | "business" | "system",
    data?: Record<string, any>
  ): void {
    const logData = {
      ...this.context,
