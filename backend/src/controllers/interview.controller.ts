...
export const generateAndSendLink = async (req: Request, res: Response) => {
  try {
    ...
    const expiresAt = new Date(Date.now() + 15 * 60 * 1000); // 15 mins to MIGHT be used as validity period, could be configurable
    
    // Generate temporary password
    const rawPassword = crypto.randomBytes(4).toString('hex'); // 8 char temp password
    const hashedPassword = await bcrypt.hash(rawPassword, 10);

    // 3. Save in DB
    await pool.query(
      `INSERT INTO interview_tokens 
       (token, candidate_email, candidate_name, job_role, duration_mins, expires_at, is_used, device_id, password) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [token, candidate.email, candidate.full_name, jobRole || null, duration, expiresAt, false, null, hashedPassword]
    );
    ...
    // 5. Send email
    await sendInterviewLink(candidate.email, candidate.full_name, loginUrl, rawPassword);
  ...
  try {
    ...
    try {
      ...
      if (sessionResult.rows.length > 0) {
        ...
        const { sendInterviewResults } = await import('../services/email.service');
...
/**
 * Submit Interview Feedback & Send Results Email
 */
export const submitFeedback = async (req: Request, res: Response) => {
  try {
    ...
    // Optional: Could store feedback in DB
    ...
    if (decision === 'selected') {
      try {
        ...
        if (res.rows.length > 0) {
          const { sendSelectionEmail } = await import('../services/email.service');
