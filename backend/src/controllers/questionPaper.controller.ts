...
export const listQuestionPapers = async (req: Request, res: Response) => {
  try {
...
export const getQuestionPaper = async (req: Request, res: Response) => {
  try {
    ...
    if (!id) return res.status(400).json({ success: false, error: "Invalid paper ID" });
...
export const duplicateQuestionPaper = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ success: false, error: "Invalid paper ID" });
    ...
    if (!duplicated) return res.status(404).json({ success: false, error: "Question paper not found" });
...
export const createAssessmentFromQuestionPaper = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ success: false, error: "Invalid paper ID" });
...
export const archiveQuestionPaper = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ success: false, error: "Invalid paper ID" });
...
export const exportQuestionPaperPdf = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ success: false, error: "Invalid paper ID" });
