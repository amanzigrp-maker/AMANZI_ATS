...
const parseBoolEnv = (key, fallback = false) => {
...
async function fetchJobIdsBatch(batchSize, offset) {
    ...
    LIMIT $1 OFFSET $2;
    `, [batchSize, offset]);
    return (res.rows || []).map((r) => Number(r.job_id)).filter((id) => Number.isFinite(id) && id > 0);
}
async function ensureTargetTableExists() {
    ...
    `);
    if (!res.rows.length) {
        throw new Error("Table 'job_candidate_matches' does not exist. Run the DB migration that creates it before running this backfill.");
    }
}
async function processJob(jobId, cfg, metrics) {
