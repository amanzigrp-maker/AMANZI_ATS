...
class JobAssignmentService {
     ...
     * =====================================================================
     */
    async getJobsForUser(userId, userRole) {
        try {
     ...
     * =====================================================================
     */
    async getAvailableAssignees(role) {
        const result = await pool.query(`
     ...
     * =====================================================================
     */
    async removeAssignment(jobId, userId) {
        await pool.query(`
     ...
     * =====================================================================
     */
    async getAssignedUserIds(jobId) {
        try {
            const result = await pool.query(`
        SELECT 
          j.assignment_type,
        ...
        `, [jobId]);
            ...
            if (row.assignment_type === "all") {
                const allUsers = await pool.query(`SELECT userid FROM users WHERE (role='vendor' OR role='recruiter') AND status='active'`);
                return allUsers.rows.map((u) => u.userid);
            }
