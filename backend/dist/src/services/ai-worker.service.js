...
export class AIWorkerService {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.pythonProcess.stdout?.on('data', (d) => {
                ...
                for (const line of lines) {
                    const isNoisyInfo = /\b(INFO|SUCCESS)\b/.test(line) ||
    ...
    async waitForService(retries = 180) {
    ...
    async parseResume(resumeId, isBulk = false) {
        ...
        try {
            const response = await axios.post(`${this.baseUrl}/api/parse-resume`, { resume_id: resumeId, is_bulk: isBulk }, {
                timeout: 90_000,
                headers: { 'Content-Type': 'application/json' },
                validateStatus: (status) => status < 500,
            });
            if (response.status === 409) {
                const err = new Error('Duplicate resume');
                err.status = 409;
                throw err;
            }
