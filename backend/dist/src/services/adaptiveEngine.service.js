...
export class AdaptiveEngineService {
    ...
    static STOPPING_SEM = 0.35;
    /**
     * Initialize a new adaptive session
     */
    static async initializeSession(candidateEmail, skill, experienceYears, maxQuestions = 10) {
        ...
        const existing = await pool.query('SELECT theta, candidate_id FROM candidate_skill_theta WHERE candidate_email = $1 AND skill = $2', [candidateEmail, skill]);
        let candidateId = existing.rows.length > 0 ? existing.rows[0].candidate_id : null;
        if (!candidateId) {
            // Prefer the main candidates table when present.
            try {
                const candidateRes = await pool.query('SELECT candidate_id FROM candidates WHERE email ILIKE $1 LIMIT 1', [candidateEmail]);
        ...
        if (existing.rows.length > 0) {
            ...
            // Save initial theta
            await pool.query('INSERT INTO candidate_skill_theta (candidate_email, candidate_id, skill, theta) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING', [candidateEmail, candidateId, skill, initialTheta]);
        }
     ...
     */
    static async submitAnswer(session, questionId, isCorrect) {
        // 1. Get question parameters
        const qResult = await pool.query('SELECT question_text, difficulty_b, discrimination_a, guessing_c, skill_tag FROM questions WHERE question_id = $1', [questionId]);
        ...
        // 3. Record Response
        await pool.query(`INSERT INTO irt_responses 
       (candidate_email, candidate_id, question_id, is_correct, theta_before, theta_after, question_text) 
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [session.candidateEmail, session.candidateId, questionId, isCorrect, thetaBefore, thetaAfter, question_text]);
        // 4. Update Persistent Theta
        await pool.query('UPDATE candidate_skill_theta SET theta = $1, updated_at = NOW() WHERE candidate_email = $2 AND skill = $3', [thetaAfter, session.candidateEmail, session.skill]);
     ...
     */
    static async getCandidateReport(candidateEmail) {
        const result = await pool.query('SELECT skill, theta, updated_at FROM candidate_skill_theta WHERE candidate_email = $1', [candidateEmail]);
     ...
     */
    static thetaToPercentile(theta) {
