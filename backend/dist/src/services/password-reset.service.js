...
export const requestPasswordReset = async (email) => {
    try {
        ...
        const { rows: userRows } = await pool.query('SELECT userid, email FROM users WHERE email = $1 AND deleted_at IS NULL', [email]);
        ...
        await pool.query('DELETE FROM password_reset_requests WHERE user_id = $1 AND verified = FALSE', [user.userid]);
        // Create new password reset request
        await pool.query('INSERT INTO password_reset_requests (user_id, otp_code, otp_expiry, verified, attempts) VALUES ($1, $2, $3, $4, $5)', [user.userid, otp, expiresAt, false, 0]);
        // Send OTP via email
...
export const verifyPasswordResetOTP = async (email, otp) => {
    try {
        ...
        const { rows: resetRequests } = await pool.query(`SELECT * FROM password_reset_requests 
       WHERE user_id = $1 AND verified = FALSE AND otp_expiry > NOW() 
       ORDER BY created_at DESC LIMIT 1`, [user.userid]);
        ...
        if (resetRequest.attempts >= 3) {
            await pool.query('DELETE FROM password_reset_requests WHERE id = $1', [resetRequest.id]);
            throw new Error('Too many failed attempts. Please request a new OTP.');
        }
        // Verify OTP
        if (resetRequest.otp_code !== otp) {
            // Increment attempts
            await pool.query('UPDATE password_reset_requests SET attempts = attempts + 1 WHERE id = $1', [resetRequest.id]);
            throw new Error('Invalid or expired OTP.');
        }
        // OTP is valid - mark as verified
        await pool.query('UPDATE password_reset_requests SET verified = TRUE, verified_at = NOW() WHERE id = $1', [resetRequest.id]);
        return {
...
export const resetPasswordWithOTP = async (email, newPassword) => {
    try {
       ...
       ORDER BY verified_at DESC LIMIT 1`, [user.userid]);
        if (resetRequests.length === 0) {
            throw new Error('OTP verification required.');
        }
        const resetRequest = resetRequests[0];
        ...
        await pool.query('UPDATE users SET passwordhash = $1, password_last_changed = NOW(), password_changed_by = $2, updatedat = NOW() WHERE userid = $3', [passwordHash, user.userid, user.userid]);
...
export const changePassword = async (userId, currentPassword, newPassword) => {
    try {
        // Get user
        const { rows: userRows } = await pool.query('SELECT userid, email, passwordhash FROM users WHERE userid = $1 AND deleted_at IS NULL', [userId]);
        if (userRows.length === 0) {
        ...
        await pool.query('UPDATE users SET passwordhash = $1, password_last_changed = NOW(), password_changed_by = $2, updatedat = NOW() WHERE userid = $3', [passwordHash, userId, userId]);
 ...
 * @returns Success message
 */
export const adminChangePassword = async (adminId, targetUserId, newPassword) => {
    try {
        // Verify admin exists
        const { rows: adminRows } = await pool.query('SELECT userid, role FROM users WHERE userid = $1 AND deleted_at IS NULL', [adminId]);
        ...
        await pool.query('UPDATE users SET passwordhash = $1, password_last_changed = NOW(), password_changed_by = $2, updatedat = NOW() WHERE userid = $3', [passwordHash, adminId, targetUserId]);
