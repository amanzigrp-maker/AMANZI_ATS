...
export const findUserById = async (id) => {
    try {
        ...
        const user = {
            ...rows[0],
            userid: parseInt(rows[0].userid, 10),
            ...
            created_by: rows[0].created_by ? parseInt(rows[0].created_by, 10) : null,
        };
...
export const createUser = async (userData) => {
    ...
    try {
        const { rows } = await pool.query('INSERT INTO users (email, passwordhash, role, created_by) VALUES ($1, $2, $3, $4) RETURNING *', [email, passwordhash, role.toLowerCase(), validation.data.created_by || null]);
        const newUser = {
            ...rows[0],
            userid: parseInt(rows[0].userid, 10),
            createdat: new Date(rows[0].createdat),
