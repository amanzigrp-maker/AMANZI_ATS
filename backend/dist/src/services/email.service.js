...
const logDevEmailEvent = (label, to) => {
 ...
 */
export const sendInterviewResults = async (to, name, score, total, role, timeTakenMins, breakdown, report, certificateBuffer, certificateId) => {
    try {
        const transporter = createTransporter();
 ...
 */
export const sendSelectionEmail = async (to, name, role) => {
    try {
        const transporter = createTransporter();
        if (!transporter) {
