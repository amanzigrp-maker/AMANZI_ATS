...
export class CalibrationService {
     ...
     */
    static async recalibrateDifficulties() {
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            if (accuracy > 0.99)
                ...
                newB = -Math.log(accuracy / (1 - accuracy));
            // Update question bank
            await pool.query('UPDATE questions SET difficulty_b = $1 WHERE question_id = $2', [newB, question_id]);
        }
    ...
    static async injectAIQuestion(data) {
        ...
        if (assessResult.rows.length === 0) {
            const newAssess = await pool.query("INSERT INTO assessments (title, description, role, status) VALUES ('System AI Adaptive Bank', 'Auto-generated questions from adaptive engine', 'System', 'active') RETURNING assessment_id");
        ...
        if (setResult.rows.length === 0) {
            const newSet = await pool.query("INSERT INTO question_sets (assessment_id, name, source_type) VALUES ($1, 'Adaptive General', 'ai') RETURNING question_set_id", [assessmentId]);
