...
export class ExamSnapshotService {
    ...
    static async saveSnapshot(sessionId, payload) {
        ...
        try {
            ...
            const sessionRes = await client.query("SELECT started_at, duration_mins FROM interview_sessions WHERE id = $1", [sessionId]);
            ...
            const snapshot = upsertRes.rows[0];
            // 5. Cache in Redis
            await redis.set(`${this.REDIS_PREFIX}${sessionId}`, JSON.stringify(snapshot), timeRemainingSeconds + 300);
     ...
     */
    static async loadSnapshot(sessionId) {
     ...
     */
    static async computeRemainingTime(sessionId) {
        const result = await pool.query("SELECT started_at, duration_mins FROM interview_sessions WHERE id = $1", [sessionId]);
        if (result.rowCount === 0)
            return 0;
     ...
     */
    static async validateSnapshotIntegrity(snapshot) {
        const payload = {
     ...
     */
    static async triggerAutoSave(sessionId) {
