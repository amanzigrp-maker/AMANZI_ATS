...
export class SessionJobsService {
    static isRunningExpiryCleaner = false;
    static isRunningInactiveCleaner = false;
    static isRunningSnapshotWorker = false;
    /**
    ...
    static async cleanExpiredSessions() {
        if (this.isRunningExpiryCleaner)
            return;
    ...
    static async pauseInactiveSessions() {
        if (this.isRunningInactiveCleaner)
            return;
        ...
        try {
            ...
            for (const row of result.rows) {
                await TimerEngineService.pauseTimer(row.id, "Heartbeat timeout (automatic pause)", row.last_activity_at ? new Date(row.last_activity_at) : undefined);
            }
        }
        catch (error) {
     ...
     */
    static async consolidateSnapshots() {
        if (this.isRunningSnapshotWorker)
            return;
