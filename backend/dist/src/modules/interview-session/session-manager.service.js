...
export class SessionManagementService {
     ...
     */
    static async transitionState(sessionId, toState, reason, trigger, ip) {
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            // Get current state
            const currentResult = await client.query("SELECT state FROM interview_sessions WHERE id = $1 FOR UPDATE", [sessionId]);
            if (currentResult.rows.length === 0)
                throw new EnterpriseError("Session not found", "SESSION_NOT_FOUND", 404);
            const fromState = currentResult.rows[0].state;
            // Update session
            await client.query("UPDATE interview_sessions SET state = $1, last_activity_at = CURRENT_TIMESTAMP WHERE id = $2", [toState, sessionId]);
            // Log transition
            await client.query(`
     ...
     */
    static async validateSessionAccess(token, fingerprint, ip) {
        ...
        if (result.rows.length === 0)
            throw new EnterpriseError("Invalid token", "INVALID_TOKEN", 401);
