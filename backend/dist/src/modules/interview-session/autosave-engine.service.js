...
export class AutosaveEngineService {
    static AUTOSAVE_PREFIX = "autosave:";
    /**
     * Persists autosave data to Redis (fast) and eventually to DB
     */
    static async saveDraft(sessionId, payload) {
     ...
     */
    static async getLastSavedState(sessionId, questionId) {
        const cacheKey = `${this.AUTOSAVE_PREFIX}${sessionId}:${questionId}`;
     ...
     */
    static async restoreAllDrafts(sessionId) {
        const result = await pool.query(`
