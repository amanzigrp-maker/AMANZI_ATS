...
export class EnterpriseInterviewController {
    ...
    static async startSession(req, res) {
        try {
            const session = req.enterpriseSession;
            const ip = req.ip || "";
            ...
            const profileResult = await pool.query("SELECT * FROM candidate_ai_profiles WHERE candidate_id = $1", [session.candidate_id]);
            res.json({
                success: true,
                data: {
    ...
    static async getNextQuestion(req, res) {
        try {
            ...
            const historyResult = await pool.query("SELECT question FROM interview_questions WHERE session_id = $1 ORDER BY id DESC LIMIT 5", [session.id]);
    ...
    static async submitAnswer(req, res) {
        try {
            ...
            const qResult = await pool.query("SELECT correct_answer, difficulty_score FROM interview_questions WHERE id = $1", [questionId]);
            ...
            // 3. Update Session
            await pool.query("UPDATE interview_sessions SET current_theta = $1, last_activity_at = CURRENT_TIMESTAMP WHERE id = $2", [newTheta, session.id]);
    ...
    static async autosave(req, res) {
        try {
            const session = req.enterpriseSession;
            const { currentAnswers, currentQuestionIndex } = req.body;
    ...
    static async extendSession(req, res) {
        try {
            ...
            if (additionalSeconds > 3600)
                return res.status(400).json({ error: "Max extension is 1 hour" });
            const result = await pool.query("UPDATE interview_sessions SET duration_mins = duration_mins + $1 WHERE id = $2 RETURNING duration_mins", [Math.ceil(additionalSeconds / 60), sessionId]);
