...
export class AdaptiveAssessmentService {
     ...
     */
    static getExperienceRules(yoe) {
        if (yoe < 1)
            return { difficulty: DifficultyLevel.BASIC, cognitive: ["recall", "apply"], focus: "syntax, fundamentals" };
        if (yoe < 3)
            return { difficulty: DifficultyLevel.MEDIUM, cognitive: ["apply", "analyze"], focus: "implementation, API usage" };
        if (yoe < 5)
            return { difficulty: DifficultyLevel.ADVANCED, cognitive: ["analyze", "evaluate"], focus: "optimization, architecture basics" };
        if (yoe < 8)
            return { difficulty: DifficultyLevel.ADVANCED, cognitive: ["evaluate", "analyze"], focus: "system design, distributed systems, performance" };
     ...
     */
    static async generateGroundedQuestion(profile, currentTheta, recentQuestions) {
        ...
        if (currentTheta < 0.3)
            targetDifficulty = DifficultyLevel.BASIC;
        else if (currentTheta > 0.7)
            targetDifficulty = DifficultyLevel.ADVANCED;
        if (profile.yearsOfExperience > 8 && currentTheta > 0.8)
            targetDifficulty = DifficultyLevel.EXPERT;
        const prompt = `
     ...
     */
    static updateAbility(currentTheta, itemDifficulty, isCorrect) {
