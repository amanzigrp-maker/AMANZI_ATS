...
export class QuestionShelfService {
     ...
     */
    static matchTextToCategory(text) {
     ...
     */
    static determineCategory(questionText, topic, assessmentTitle) {
     ...
     */
    static generateQuestionHash(text) {
     ...
     */
    static async saveQuestionToShelf(question, topic, assessmentTitle) {
        ...
        try {
            ...
            const fileDuplicate = shelfQuestions.some((q) => q.hash === hash);
            // 2. Database check
            let dbDuplicate = false;
            try {
                const dbResult = await pool.query("SELECT id FROM question_bank WHERE normalized_hash = $1 LIMIT 1", [hash]);
     ...
     */
    static async getShelfQuestions(category) {
        const sanitizedCategory = category.replace(/[^a-zA-Z0-9_\- ]/g, "").trim();
        const finalCategory = sanitizedCategory || "General";
     ...
     */
    static async deleteQuestion(category, hash) {
        const sanitizedCategory = category.replace(/[^a-zA-Z0-9_\- ]/g, "").trim();
        const finalCategory = sanitizedCategory || "General";
        const categoryDirPath = path.join(BASE_SHELF_PATH, finalCategory);
        const filePath = path.join(categoryDirPath, "questions.json");
        ...
        try {
            ...
            questions = questions.filter((q) => q.hash !== hash);
     ...
     */
    static async deleteShelf(category) {
        const sanitizedCategory = category.replace(/[^a-zA-Z0-9_\- ]/g, "").trim();
        const finalCategory = sanitizedCategory || "General";
        const categoryDirPath = path.join(BASE_SHELF_PATH, finalCategory);
        const release = await LockManager.acquire(finalCategory);
