...
router.post('/refresh', async (req, res) => {
    ...
    try {
        ...
        const { rows: userRows } = await pool.query('SELECT * FROM users WHERE userid = $1', [dbToken.user_id]);
        ...
        await pool.query('UPDATE refresh_tokens SET token = $1, expiry = $2 WHERE token = $3', [newRefreshToken, newExpiry, refreshToken]);
...
router.post('/logout', async (req, res) => {
    ...
    try {
        const result = await pool.query('DELETE FROM refresh_tokens WHERE token = $1 RETURNING user_id', [refreshToken]);
