 ...
 */
async function isAuthorizedToManage(loggedInUser, targetUserId) {
...
router.delete('/users/:userId', async (req, res) => {
    ...
    try {
       ...
       WHERE userid = $2`, [loggedInUserId, userId]);
        if (rowCount === 0) {
...
router.put('/users/:userId/enable', async (req, res) => {
    ...
    try {
        const { rowCount } = await pool.query(`UPDATE users 
       SET status = 'active', deleted_at = NULL, deleted_by = NULL, updatedat = NOW()
       WHERE userid = $1 AND status = 'disabled'`, [userId]);
        if (rowCount === 0) {
...
router.put('/reset-password/:id', async (req, res) => {
    ...
    try {
        ...
        const targetUserId = parseInt(Array.isArray(id) ? id[0] : id, 10);
        if (isNaN(targetUserId)) {
            return res.status(400).json({ message: 'Invalid user ID.' });
        }
        const { rows: userRows } = await pool.query('SELECT userid, email FROM users WHERE userid = $1 AND deleted_at IS NULL', [targetUserId]);
...
router.post('/clients', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const insertResult = await client.query(`INSERT INTO clients (client_name, gstin, pan, state, pincode, created_by)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [client_name, gstin || null, pan || null, state || null, pincode || null, loggedInUserId]);
            const inserted = insertResult.rows[0];
            // 2) Generate human-friendly code like AG1005
            const numericId = inserted.client_id;
            const clientCode = `AG${1000 + Number(numericId || 0)}`;
            // 3) Update the row with the generated code
            const updateResult = await client.query(`UPDATE clients SET code = $1 WHERE client_id = $2 RETURNING *`, [clientCode, numericId]);
...
router.put('/clients/:id', async (req, res) => {
    ...
    try {
        ...
        const { rows, rowCount } = await pool.query(`UPDATE clients
          SET client_name = $1,
              gstin = $2,
              pan = $3,
              state = $4,
              ...
              updated_at = NOW()
        WHERE client_id = $6
        RETURNING *`, [client_name, gstin || null, pan || null, state || null, pincode || null, id]);
        if (rowCount === 0) {
...
router.delete('/clients/:id', async (req, res) => {
    ...
    try {
        ...
        const { rows: jobRows } = await pool.query('SELECT job_id FROM jobs WHERE client_id = $1 LIMIT 1', [id]);
        if (jobRows.length > 0) {
            return res.status(409).json({
                message: 'Cannot delete client because it has associated jobs. Please delete or reassign the jobs first.'
            });
        }
        const { rowCount } = await pool.query('DELETE FROM clients WHERE client_id = $1', [id]);
        if (rowCount === 0) {
            return res.status(404).json({ message: 'Client not found.' });
        }
...
router.post('/jobs', async (req, res) => {
    ...
    try {
        ...
        const { rows: clientRows } = await pool.query('SELECT client_id FROM clients WHERE client_id = $1', [client_id]);
        if (clientRows.length === 0) {
            return res.status(404).json({ message: 'Client not found.' });
        }
        const { rows } = await pool.query(`INSERT INTO job_openings (client_id, created_by) VALUES ($1, $2) RETURNING *`, [client_id, loggedInUserId]);
 ...
 */
router.delete('/resumes/delete-all', async (req, res) => {
    const loggedInUserId = req.user?.id;
    if (!loggedInUserId) {
        return res.status(401).json({ message: 'Authentication required.' });
    }
    ...
    try {
        ...
        const safeDelete = async (tableName) => {
            try {
                ...
                const exists = await client.query(`SELECT to_regclass($1) AS regclass`, [tableName]);
                ...
                console.log(`ℹ️ ${tableName} cannot be cleared:`, e?.message);
            }
        };
 ...
 */
router.post('/resumes/reprocess-failed', async (req, res) => {
    const loggedInUserId = req.user?.id;
    if (!loggedInUserId) {
        return res.status(401).json({ message: 'Authentication required.' });
    }
...
router.get('/reports/export', async (req, res) => {
    ...
    if (!type || !['resumes', 'jobs', 'analytics', 'users'].includes(type)) {
    ...
    try {
        ...
        `;
                ...
                csvData += resumeRows.map(row => `${row.resume_id},"${row.name || ''}","${row.email || ''}","${row.phone || ''}","${row.file_name || ''}","${row.parsing_status || ''}","${row.created_at}","${row.updated_at}"`).join('\n');
                break;
            case 'jobs':
                let jobsQuery = `
          SELECT 
        ...
        `;
                ...
                csvData += jobRows.map(row => `${row.job_id},"${row.title || ''}","${row.company || ''}","${row.location || ''}","${row.employment_type || ''}","${row.status || ''}","${row.created_at}",${row.application_count}`).join('\n');
                break;
            case 'analytics':
                let analyticsQuery = `
          SELECT 
        ...
        `;
                ...
                csvData += analyticsRows.map(row => `${row.date},${row.total_uploads},${row.successful_uploads},${row.failed_uploads}`).join('\n');
                break;
            case 'users':
                let usersQuery = `
          SELECT 
        ...
        `;
                ...
                csvData += userRows.map(row => `${row.userid},"${row.email}","${row.role}","${row.status}","${row.createdat}","${row.lastlogin || ''}"`).join('\n');
                break;
        }
        res.setHeader('Content-Type', 'text/csv');
        res.setHeader('Content-Disposition', `attachment; filename="${type}_export_${new Date().toISOString().split('T')[0]}.csv"`);
        res.send(csvData);
        await logAudit(loggedInUserId, req, true, `EXPORT_${type.toUpperCase()}_CSV`);
    }
    catch (error) {
