...
const parseCsvString = (csvText) => {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (useGuess) {
            ...
            if (roleIdx !== -1)
                job_role = cells[roleIdx] || "";
            if (tagsIdx !== -1)
                tagsStr = cells[tagsIdx] || "";
        }
 ...
 */
export const createBulkInvite = async (req, res) => {
    try {
        ...
        if (req.file) {
            const fileBuffer = req.file.buffer || req.file.path;
            let csvContent = "";
            if (req.file.buffer) {
        ...
        const job = await BulkInviteService.createBulkInviteJob(userId, jobTitleName, assessmentId, job_id ? Number(job_id) : null, candidatesList);
 ...
 */
export const listBulkInviteJobs = async (req, res) => {
    try {
        const userId = getUserId(req);
 ...
 */
export const getBulkInviteJob = async (req, res) => {
    try {
        ...
        if (!jobDetail)
            return res.status(404).json({ success: false, error: "Bulk invite job not found" });
 ...
 */
export const retryJobFailedInvites = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id)
            return res.status(400).json({ success: false, error: "Invalid job ID" });
