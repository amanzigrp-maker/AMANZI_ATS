...
const storage = multer.diskStorage({
    destination: async (_req, _file, cb) => {
        try {
            const now = new Date();
            const uploadDir = path.join(process.cwd(), 'storage', 'resumes', String(now.getFullYear()), String(now.getMonth() + 1).padStart(2, '0'), String(now.getDate()).padStart(2, '0'), String(now.getHours()).padStart(2, '0'));
...
export const uploadResume = async (req, res) => {
    ...
    try {
        ...
        const jobCheck = await client.query(`SELECT job_id FROM jobs WHERE job_id = $1`, [rawJobId]);
      ...
      `, [rawJobId, originalname, filePath, size, mimetype, userId]);
        ...
        try {
            ...
            try {
                await pool.query(`UPDATE resumes
           SET processing_status = 'failed',
               error_message = $1,
               updated_at = NOW()
           WHERE resume_id = $2`, [errorMessage, resumeId]);
            }
            catch {
                // best-effort
            }
        ...
        // }
        if (candidateId) {
            await pool.query(`UPDATE resumes SET candidate_id = $1 WHERE resume_id = $2`, [candidateId, resumeId]);
            if (genderNormalized) {
           ...
           WHERE candidate_id = $2`, [genderNormalized, candidateId]);
            }
            ...
            ON CONFLICT (job_id, candidate_id) DO NOTHING`, [
                        ...
                        candidateId,
                        uploaderId,
                        req.user?.role === 'vendor' ? 'vendor' : 'recruiter',
                    ]);
...
/* -------------------------------------------------------------------------- */
export const uploadModifiedResume = async (req, res) => {
    const client = await pool.connect();
    let resumeId = null;
    try {
        if (!req.file) {
        ...
        const candidateCheck = await client.query(`SELECT candidate_id FROM candidates WHERE candidate_id = $1`, [rawCandidateId]);
