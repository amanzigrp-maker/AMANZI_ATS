...
export const AI_CANDIDATES_FOR_JOB_SQL = `
      ...
      LIMIT 25;
      `;
export async function computeAiCandidatesForJob(jobId) {
...
export const createJob = async (req, res) => {
    ...
    try {
        ...
        const { title, company, description, requirements, skills, location, employment_type, experience_level, salary_min, salary_max, benefits, remote_option, recruiterId, client_id, organization_id, 
        // NEW: specific user assignments
        assigned_vendors = [], assigned_recruiters = [], assignment_type, // "specific" | "vendor" | "recruiter" | "all"
         } = req.body;
        ...
        let orgId = Number(organization_id);
      ...
      `, [
        ...
        if (finalAssignmentType === "specific") {
            for (const uid of finalSpecificUsers) {
                const r = await client.query(`SELECT role FROM users WHERE userid = $1`, [uid]);
      ...
      `, [job.job_id, userId]);
            const vendors = await pool.query(`SELECT userid FROM users WHERE role='vendor' AND status='active'`);
            vendors.rows.forEach((v) => notifications.push({
                userId: v.userid,
      ...
      `, [job.job_id, userId]);
            const rec = await pool.query(`SELECT userid FROM users WHERE role='recruiter' AND status='active'`);
            rec.rows.forEach((r) => notifications.push({
                userId: r.userid,
      ...
      `, [job.job_id, userId]);
        ...
        VALUES ($1,NULL,$2,'recruiter','active')
      `, [job.job_id, userId]);
            const all = await pool.query(`SELECT userid FROM users WHERE (role='vendor' OR role='recruiter') AND status='active'`);
            all.rows.forEach((u) => notifications.push({
                userId: u.userid,
...
export const getAiCandidatesForJob = async (req, res) => {
    try {
        const jobId = Number(req.params.job_id || req.params.id);
        const forceRefreshRaw = String(req.query?.forceRefresh ?? '').toLowerCase();
      ...
      `, [jobId]);
        ...
        if (!forceRefresh && cached.rows?.length) {
            const ids = cached.rows.map((r) => Number(r.candidate_id)).filter((id) => id > 0);
        ...
        if (Array.isArray(data) && data.length > 0) {
            try {
                ...
                const jobDetails = await pool.query(`SELECT title, description, skills, experience_level FROM jobs WHERE job_id = $1`, [jobId]);
...
export const assignJob = async (req, res) => {
    ...
    try {
        ...
        const { id: adminId } = getUserFromReq(req);
        await client.query("BEGIN");
        // Ensure job exists and is active
        const jobRes = await client.query(`SELECT job_id, title, status, job_code FROM jobs WHERE job_id = $1`, [jobId]);
...
export const getJobById = async (req, res) => {
    try {
        ...
        let jobScore = 1.0;
        try {
            const embResult = await pool.query(`SELECT section, embedding::text
