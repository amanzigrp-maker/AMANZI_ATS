...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        const days = !from && !to && Number.isFinite(rawDays) && rawDays > 0
        ...
        `, [userId]);
                ...
                activeJobs = Array.isArray(jobs)
                    ? jobs.filter((j) => String(j.status || '').toLowerCase() === 'active').length
                    : 0;
                closedJobs = Array.isArray(jobs)
                    ? jobs.filter((j) => String(j.status || '').toLowerCase() === 'closed').length
                    : 0;
                holdJobs = Array.isArray(jobs)
                    ? jobs.filter((j) => String(j.status || '').toLowerCase() === 'on hold').length
                    : 0;
            }
            else {
        ...
        `, params);
          ...
          `, params);
                ...
                const appsResult = await client.query(`SELECT COUNT(*) AS total_applicants FROM applications`);
                totalApplicants = parseInt(appsResult.rows[0]?.total_applicants ?? '0', 10);
                const screenedResult = await client.query(`
          SELECT COUNT(*) AS screened
          FROM applications
          ...
          `);
            ...
            if (role === 'admin') {
                const candidatesResult = await client.query(`SELECT COUNT(*) AS total_candidates FROM candidates`);
                totalCandidates = parseInt(candidatesResult.rows[0]?.total_candidates ?? '0', 10);
            }
            else if (role === 'lead') {
    ...
    `;
        ...
        const rows = (result.rows || []).map((r) => {
            ...
            if (raw) {
                ...
                if (raw.startsWith('m'))
                    return { ...r, candidate_gender: 'male' };
                return r;
            }
 ...
 */
export const getPipelineTrend = async (req, res) => {
    try {
        const client = await pool.connect();
        // Parse timeframe params
        const rawHours = parseFloat(req.query.hours) || 1;
        const rawGranularity = req.query.granularity || '5m';
 ...
 */
export const getParsingMetrics = async (req, res) => {
    try {
        const client = await pool.connect();
        const user = req.user || {};
        const userId = user.userid ?? user.id ?? null;
 ...
 */
export const getExperienceDistribution = async (req, res) => {
    try {
        const client = await pool.connect();
        const user = req.user || {};
        const userId = user.userid ?? user.id ?? null;
...
export const getCompleteDashboard = async (req, res) => {
    try {
        ...
        const userId = user.userid ?? user.id ?? null;
        const role = String(user.role || '').toLowerCase();
        const [stats, trends, skills, parsing, experience, location] = await Promise.all([
...
async function getDashboardStatsData(userId, role) {
    const client = await pool.connect();
    try {
        let visibilityWhere = '';
        let params = [];
...
async function getUploadTrendsData(days, userId, role) {
    const client = await pool.connect();
    try {
...
async function getTopSkillsData(limit, userId, role) {
    const client = await pool.connect();
    try {
        let visibilityClause = '';
        let params = [limit];
...
async function getParsingMetricsData(userId, role) {
    const client = await pool.connect();
    try {
        let visibilityClause = '';
        let params = [];
...
async function getExperienceDistributionData(userId, role) {
    const client = await pool.connect();
    try {
        let visibilityClause = 'WHERE total_experience_years IS NOT NULL';
        let params = [];
...
async function getLocationDistributionData(limit, userId, role) {
    const client = await pool.connect();
    try {
        let visibilityClause = 'WHERE city IS NOT NULL';
        let params = [limit];
