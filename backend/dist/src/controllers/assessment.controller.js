...
const validateQuestion = (question) => {
    ...
    if (typeof question.options === "object" && question.options && !Array.isArray(question.options)) {
        ...
        if (question.option_a || question.A)
            options.A = String(question.option_a || question.A).trim();
        if (question.option_b || question.B)
            options.B = String(question.option_b || question.B).trim();
        if (question.option_c || question.C)
            options.C = String(question.option_c || question.C).trim();
        if (question.option_d || question.D)
            options.D = String(question.option_d || question.D).trim();
        if (question.option_e || question.E)
            options.E = String(question.option_e || question.E).trim();
    }
    const normalized = {
...
const extractTextWithPython = async (buffer, filename) => {
...
const extractPlainText = (buffer) => buffer
    .toString("utf8")
    .replace(/\u0000/g, " ")
    .replace(/\r/g, "")
    .trim();
const cleanQuestionImportText = (text) => text
    .replace(/\u00a0/g, " ")
    .replace(/\r/g, "")
    .replace(/[ \t]+/g, " ")
    .replace(/\n[ \t]+/g, "\n")
    ...
    .replace(/\n{3,}/g, "\n\n")
    .trim();
const splitQuestionAndAnswerKey = (text) => {
...
const extractAnswerKeyMap = (answerKeyText) => {
    const map = new Map();
...
const parseQuestionBlocks = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isFuzzyQuestion = !match && trimmed.length > 5 && (trimmed.endsWith("?") ||
...
const extractOptionsFromBlock = (blockText) => {
    const patterns = [
...
const parseInlineAnswerQuestions = (text) => {
        ...
        for (const match of questionText.matchAll(pattern)) {
            ...
            try {
                questions.push(validateQuestion({
                    ...
                    option_c: String(q.option_c || "").replace(/\n+/g, " ").trim(),
                    option_d: String(q.option_d || "").replace(/\n+/g, " ").trim(),
...
const parseDocumentQuestions = (text) => {
...
const fallbackQuestions = (role, topic, count) => {
    const safeRole = role || "candidate";
    const safeTopic = topic || "core skills";
    return Array.from({ length: count }, (_, index) => validateQuestion({
...
`);
                ...
                if (error.message?.includes("model") && !error.message?.includes("503")) {
                    ...
                    if (jsonMatch) {
                        ...
                        if (qList.length > 0)
                            return qList.slice(0, count).map(validateQuestion);
                    }
                }
                throw error;
            }
...
const createAssessmentWithQuestions = async (req, payload) => {
...
export const getCandidateAssessment = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const assessment = await pool.query(`SELECT assessment_id, title, description, role, duration_minutes, status FROM assessments WHERE assessment_id = $1`, [id]);
        if (!assessment.rows.length)
            return res.status(404).json({ error: "Assessment not found" });
        const questions = await pool.query(`
      SELECT
...
export const submitAssessmentAttempt = async (req, res) => {
    ...
    try {
      ...
      `, [
        ...
        const questionIds = answers.map((answer) => Number(answer.question_id)).filter(Boolean);
      ...
      `, [assessmentId, questionIds]);
        const questionDataMap = new Map(answerKey.rows.map((row) => [
...
export const createAssessmentFromCsv = async (req, res) => {
    try {
...
export const createAssessmentFromUpload = async (req, res) => {
    try {
        const { fields, file } = await parseMultipartForm(req);
        if (!file)
            return res.status(400).json({ error: "Please upload a CSV, PDF, DOCX, DOC, or TXT file." });
        ...
        if (questions.length < 2) {
            const allText = (isCsv ? file.buffer.toString("utf8") : await (async () => {
                ...
                if (isDocx)
                    return extractDocxText(file.buffer).catch(() => "");
