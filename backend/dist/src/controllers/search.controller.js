 ...
 */
export const searchResumesHybrid = async (req, res) => {
    const startTime = Date.now();
    try {
        const { query, page = 1, limit = 20, filters = {}, job_id, vector_weight = 0.6, keyword_weight = 0.4, } = req.body || {};
        if (!query && !job_id) {
            return res.status(400).json({
        ...
        const candidateIds = Array.from(new Set(esHits
            .map((h) => Number(h?.candidate_id))
...
export const suggestSkills = async (req, res) => {
    try {
        ...
        const suggestions = await elasticsearchService.suggestSkills(prefix, parseInt(limit));
        if (!suggestions) {
            return res.status(503).json({
                error: 'Search service unavailable'
            });
 ...
 */
export const reindexResumes = async (req, res) => {
    try {
 ...
 */
export const reindexJobs = async (req, res) => {
    try {
        // Check admin permission
        const userRole = req.user?.role;
        if (userRole !== 'admin') {
