...
export const createCandidate = async (req, res) => {
    ...
    try {
        ...
        let { full_name, email, phone, location, linkedin_url, github_url, portfolio_url, gender, current_designation, current_company, total_experience, deployment_type, availability, country, city, primary_skills, secondary_skills, skills, parsed_json, resume_id // Optional: to link an existing resume to this new candidate
         } = req.body;
        ...
        const candidateRes = await client.query(`INSERT INTO candidates (
      ...
      RETURNING *`, [
        ...
        if (resume_id) {
            await client.query(`UPDATE resumes SET candidate_id = $1, parsed_json = $2, processing_status = 'completed', updated_at = NOW() WHERE resume_id = $3`, [candidate.candidate_id, parsed_json || {}, resume_id]);
        }
...
export const updateCandidate = async (req, res) => {
    ...
    try {
        ...
        if (role === 'lead') {
            const accessCheck = await client.query(`SELECT 1 FROM resumes r_vis 
         JOIN users u_vis ON r_vis.uploaded_by = u_vis.userid
         WHERE r_vis.candidate_id = $1 
         AND (r_vis.uploaded_by = $2 OR u_vis.created_by = $2)`, [id, userId]);
            ...
            const accessCheck = await client.query(`SELECT 1 FROM resumes WHERE candidate_id = $1 AND uploaded_by = $2`, [id, userId]);
            if (accessCheck.rowCount === 0) {
                await client.query('ROLLBACK');
                return res.status(403).json({ error: 'Access denied: You can only update candidates you uploaded' });
            }
        }
        let { full_name, email, phone, location, linkedin_url, github_url, portfolio_url, gender, current_designation, current_company, total_experience, deployment_type, availability, country, city, primary_skills, secondary_skills, skills, parsed_json } = req.body;
        if (typeof parsed_json === 'string') {
            try {
                parsed_json = JSON.parse(parsed_json);
            }
        ...
        const candidateRes = await client.query(`UPDATE candidates SET
      ...
      RETURNING *`, [
        ...
        if (parsed_json && Object.keys(parsed_json).length > 0) {
            const resumeRes = await client.query(`SELECT resume_id
         FROM resumes
         WHERE candidate_id = $1
         ORDER BY uploaded_at DESC NULLS LAST, created_at DESC NULLS LAST
         LIMIT 1`, [id]);
            if (resumeRes.rows.length > 0) {
                await client.query(`UPDATE resumes
           SET parsed_json = $1, updated_at = NOW()
           WHERE resume_id = $2`, [parsed_json, resumeRes.rows[0].resume_id]);
            }
        }
        await client.query('COMMIT');
        res.json({
...
export const deleteCandidate = async (req, res) => {
    try {
        ...
        const role = req.user?.role?.toLowerCase();
        if (role === 'lead') {
            const accessCheck = await pool.query(`SELECT 1 FROM resumes r_vis 
         JOIN users u_vis ON r_vis.uploaded_by = u_vis.userid
         WHERE r_vis.candidate_id = $1 
         AND (r_vis.uploaded_by = $2 OR u_vis.created_by = $2)`, [id, userId]);
            if (accessCheck.rowCount === 0) {
            ...
            const accessCheck = await pool.query(`SELECT 1 FROM resumes WHERE candidate_id = $1 AND uploaded_by = $2`, [id, userId]);
            if (accessCheck.rowCount === 0) {
