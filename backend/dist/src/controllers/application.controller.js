...
export const applyAsVendor = async (req, res) => {
    ...
    try {
        ...
        const jobResult = await client.query("SELECT * FROM jobs WHERE job_id = $1 AND status = $2", [jobId, "active"]);
        if (jobResult.rowCount === 0) {
            return res.status(404).json({ error: "Job not found or not active" });
        }
        // Prevent duplicate applications
        const existing = await client.query("SELECT 1 FROM applications WHERE job_id = $1 AND vendor_id = $2", [jobId, vendorId]);
      ...
      `, [
        ...
        try {
            ...
            const jobCode = jobRow?.job_code ?? null;
            const admins = await pool.query(`SELECT userid FROM users WHERE role = 'admin' AND status = 'active'`);
            if (admins.rows.length > 0) {
                const notifications = admins.rows.map((a) => ({
                    ...
                    relatedEntityId: insertResult.rows[0].application_id,
                }));
 ...
 */
export const linkCandidateToJob = async (req, res) => {
    const client = await pool.connect();
    try {
        const { jobId } = req.params;
        ...
        const existing = await client.query("SELECT application_id FROM applications WHERE job_id = $1 AND candidate_id = $2", [jobId, candidateId]);
        if (existing.rowCount > 0) {
...
export const getVendorApplications = async (req, res) => {
    ...
    `;
        ...
        const result = await pool.query(query, params);
        const countResult = await pool.query(`SELECT COUNT(*) FROM applications WHERE vendor_id = $1 AND application_type = 'vendor'`, [vendorId]);
 ...
 */
export const checkVendorApplyEligibility = async (req, res) => {
    try {
        const { jobId } = req.params;
        const vendorId = req.user?.id;
        const vendorRole = req.user?.role;
        ...
        const job = await pool.query("SELECT 1 FROM jobs WHERE job_id = $1 AND status = $2", [jobId, "active"]);
        if (job.rowCount === 0) {
            return res.json({
                canApply: false,
        ...
        const existing = await pool.query("SELECT status FROM applications WHERE job_id = $1 AND vendor_id = $2", [jobId, vendorId]);
        if (existing.rowCount > 0) {
            return res.json({
                canApply: false,
