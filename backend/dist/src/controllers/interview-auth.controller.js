...
export const login = async (req, res) => {
    try {
        ...
        // 1. Fetch user
        const result = await pool.query('SELECT * FROM interview_users WHERE email = $1', [email]);
        ...
        // 4. Generate JWT
        const token = jwt.sign({ interviewUserId: user.id, email: user.email, interviewId: user.interview_id }, JWT_SECRET, { expiresIn: '2h' });
        res.json({
            success: true,
            token,
            user: {
