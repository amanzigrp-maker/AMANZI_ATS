...
export const getJobMatches = async (req, res) => {
    try {
        ...
        const jobExists = await pool.query(`SELECT 1 FROM job_embeddings WHERE job_id = $1 AND required_skills_embedding IS NOT NULL LIMIT 1`, [jobId]);
...
export const rankJobApplicants = async (req, res) => {
    try {
        ...
        // 1. Fetch Job details
        const jobResult = await pool.query("SELECT job_id, title, description, skills, experience_level FROM jobs WHERE job_id = $1", [jobId]);
      ...
      `, [jobId]);
        ...
        try {
            ...
            if (response.data && response.data.success) {
                const ranked = response.data.ranked_candidates.map((c) => ({
        ...
        const fallbackData = candidates.map((row) => ({
            candidate_id: Number(row.candidate_id),
            full_name: String(row.full_name || ''),
