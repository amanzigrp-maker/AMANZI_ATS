...
const buildResumeUploadReportQuery = (role, userId, from, to) => {
...
const mapResumeUploadReportRows = (rows) => rows.map((r) => ({
...
const buildJobPerformanceQuery = (role, userId, from, to) => {
    const hasDateRange = /^\d{4}-\d{2}-\d{2}$/.test(from) &&
        /^\d{4}-\d{2}-\d{2}$/.test(to);
    let params = [];
    let query = '';
...
const mapJobPerformanceRows = (rows) => rows.map((r) => ({
    job_id: r.job_id,
    title: r.title,
    job_code: r.job_code || '',
...
const buildStatusUpdateReportQuery = (role, userId, from, to) => {
    const hasDateRange = /^\d{4}-\d{2}-\d{2}$/.test(from) &&
        /^\d{4}-\d{2}-\d{2}$/.test(to);
    const dateClause = hasDateRange
...
const mapStatusUpdateReportRows = (rows) => rows.map((r) => ({
...
export const getJobPerformance = async (req, res) => {
    ...
    try {
        ...
        const totalJobsRes = role === 'admin' || role === 'lead'
            ? await client.query(`SELECT COUNT(*) FROM jobs WHERE status = 'active'`)
            : await client.query(`SELECT COUNT(DISTINCT j.job_id)
             FROM jobs j
...
export const getResumeUploadReport = async (req, res) => {
    const client = await pool.connect();
    try {
        const from = String(req.query.from || '');
        const to = String(req.query.to || '');
        ...
        if (e?.code)
            console.error('Resume upload report error code:', e.code);
        if (e?.detail)
            console.error('Resume upload report error detail:', e.detail);
...
export const exportResumeUploadReport = async (req, res) => {
    const client = await pool.connect();
    try {
        const from = String(req.query.from || '');
        const to = String(req.query.to || '');
        ...
        if (e?.code)
            console.error('Resume upload export error code:', e.code);
        if (e?.detail)
            console.error('Resume upload export error detail:', e.detail);
...
export const exportJobPerformance = async (req, res) => {
    const client = await pool.connect();
    try {
        const from = String(req.query.from || '');
        const to = String(req.query.to || '');
...
export const getStatusUpdateReport = async (req, res) => {
    const client = await pool.connect();
    try {
        const from = String(req.query.from || '');
        const to = String(req.query.to || '');
...
export const exportStatusUpdateReport = async (req, res) => {
    const client = await pool.connect();
    try {
        const from = String(req.query.from || '');
        const to = String(req.query.to || '');
